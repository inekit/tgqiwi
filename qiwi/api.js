...
module.exports = function Qiwi(token, proxyString) {
  let config = { timeout: 3000 };
  if (proxyString) {
    var agent = new httpsProxyAgent(proxyString);
  ...
  this.getAccountLimits = function (wallet, requestOptions) {
    ...
    const params = requestOptions
      .map((option, idx) => `types%5B${idx}%5D=${option}`)
      .join("&");
    const options = {
   ...
   * @param {[string]} requestOptions
   */
  this.getOutLimits = function (wallet) {
    // TODO: Use params serializer - https://stackoverflow.com/questions/49944387/how-to-correctly-use-axios-params-with-arrays
    const options = {
      url: `${apiUri}/person-profile/v1/persons/${wallet}/status/restrictions`,
    };
   ...
   */
  this.checkOnlineCommission = async function (card, requestOptions) {
    var card = await detectCard(card);

    var options = {
      url: `${apiUri}/sinap/providers/${card.message}/onlineCommission`,
      body: {
        account: requestOptions.account,
        paymentMethod: {
          type: "Account",
    ...
    var options = {
      ...
      params: {
        ...
        min_creation_datetime:
          requestOptions.from == null ? null : requestOptions.from.getTime(),
        max_creation_datetime:
          requestOptions.to == null ? null : requestOptions.to.getTime(),
        next_id: requestOptions.nextId,
        next_creation_datetime:
          requestOptions.nextDate == null
            ? null
            : requestOptions.nextDate.getTime(),
      },
    };
  ...
  async function detectOperator(phone) {
    ...
    try {
      ...
      if (result.code.value == "2") throw new Error(errorMessage);
      return result;
    } catch (error) {
  ...
  async function get(options) {
    ...
    try {
      ...
      if (result.data.errorCode != undefined) return;
  ...
  async function patch(options) {
    ...
    try {
      ...
      if (result.data.errorCode != undefined) throw result.data;
